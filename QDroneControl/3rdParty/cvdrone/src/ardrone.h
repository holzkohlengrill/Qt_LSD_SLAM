...
public:
    ...
    // Move ONCE (whatever it means) with velocity [m/s]
    virtual void move(double vx, double vy, double vr);
    virtual void move3D(double vx, double vy, double vz, double vr);

    // Start moving with velocity, set 0 to stop
    virtual void setVelocity(ControlCommand &c);
...
protected:
    ...
    //control velocity for continuous movement - BC
    ControlCommand controlVelocity;
